...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
     ...
     * entry value. */
     pgdir_entryp = pgdir + PDX(va);	//閾忔艾婀撮崸鈧?
     ...
     * table. */
     if( (*pgdir_entryp & PTE_V) ==0 ){
     	if(create==1)
		*pgdir_entryp = PADDR(alloc(BY2PG,BY2PG,1))|PTE_V|PTE_R;//alloc 鏉╂柨娲栭搹姘勾閸р偓	
	else return 0;
     }
     
    /* Step 3: Get the page table entry for `va`, and return it. */
     pgtable = KADDR(VA2PFN(*pgdir_entryp)); //妞や絻銆冩径纾嬫珓閸︽澘娼?
     pgtable_entry = pgtable + PTX(va);  // 妞や絻銆冩い纭呮珓閸︽澘娼?
    // printf("\n[%x %x %x]\n",pgdir,pgtable,pgtable_entry);
...
page_init(void)
{
     ...
     * filed to 1) */
     for(i=0;page2kva(&pages[i]) < (void*)freemem;i++){
     	pages[i].pp_ref = 1;
     }
     //printf("%d %d %d\n",i,npage,sizeof(struct Page));
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
    ...
    pgtable = KADDR(VA2PFN(*pgdir_entryp));
...
page_insert(Pde *pgdir, struct Page *pp, u_long va, u_int perm)
{
    ...
    //printf("page_insert_check :%x\n",va);
    pgdir_walk(pgdir, va, 0, &pgtable_entry); //閼板啳妾荤€甸€涚艾娴滃瞼楠囨い浣冦€冨鎻掔摠閸︼拷
    ...
    //printf("pde_entry is: %x\n",*(pgdir+PDX(va)));
    //閼板啳妾绘禍宀€楠囨い浣冦€冩稉宥呯摠閸︼拷
    /* Step 3.1 Check if the page can be insert, if can閳ユ獩 return -E_NO_MEM */
    if(x!=0) return -E_NO_MEM;
