...
#define LIST_INSERT_TAIL(head, elm, field) do {                      \
		if(LIST_FIRST(head)==NULL) LIST_INSERT_HEAD(head,elm,field);  \
			 ...
			 typeof(LIST_FIRST(head)) mpage=LIST_FIRST(head);  \
			while(LIST_NEXT(mpage,field)!= NULL){ mpage = LIST_NEXT(mpage,field);}   \
			 LIST_INSERT_AFTER(mpage,elm,field);        \
			 ...
			 LIST_INSERT_AFTER(LIST_NEXT(elm,field),elm,field); \
			 */\
	    	}\
	\
      } while  (0) 
