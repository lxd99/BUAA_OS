...
pipe(int pfd[2])
{
	...
	//writef("pageref at %x:%x %x:%x \n",fd2data(fd0),pageref(fd2data(fd0)),fd2data(fd1),pageref(fd2data(fd1)));
	//writef("pageref at %x:%x %x:%x \n",fd0,pageref(fd0),fd1,pageref(fd1));
...
_pipeisclosed(struct Fd *fd, struct Pipe *p)
{
	...
	while(1){
		runs = env->env_runs;
		ret = pageref(p) == pageref(fd) ?1:0;
		if(runs != env->env_runs) continue;
		/*if(ret == 1){
			writef("id is %x ,runs is %x ref is %x\n",syscall_getenvid(),runs,pageref(p));
		}*/
		return ret; 
	}
...
pipewrite(struct Fd *fd, const void *vbuf, u_int n, u_int offset)
{
	...
	wbuf = vbuf;
	for(i=0;i<n;i++){
		while((p->p_wpos - p->p_rpos) >= BY2FILE){
