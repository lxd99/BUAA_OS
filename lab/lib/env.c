...
env_setup_vm(struct Env *e)
{
     ...
     */
     for(;i<1024;i++) pgdir[i] = boot_pgdir[i];
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
	...
	printf("check_load_va is %x ",dva+i);
	printf("check_load_should_be:%x\n",page2pa(p));
    }
...
//    printf("set_context\n");
    ...
    //printf("check:%x \n",dick);
    //pgdir_walk(curenv->env_pgdir,0x400000,0,&dick1); 
    //printf("%x \n",*dick1);
    //printf("%x\n",*dick1);
